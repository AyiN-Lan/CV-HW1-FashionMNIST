...
class MLP:
    def __init__(self,hidden_dim=128,activation=ACTIVATION):
        if activation=="relu":
            self.act=relu
    ...
    for hd in search_space["hidden_dim"]:
        for lr in search_space["learning_rate"]:
            for l2 in search_space["lambda_l2"]:
                ...
                model=MLP(hidden_dim=hd,activation=ACTIVATION)
...
if __name__=="__main__":
    ...
    final_model=MLP(hidden_dim=best_params["hidden_dim"],activation=ACTIVATION)
